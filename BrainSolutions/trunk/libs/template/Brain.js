...
Brain = {
    ...
    sWidth: null,
    sHeight: null,
    appLoadMessage: "CONTINUE",
    landscape: false,
    assessTimer: 0,
    
    // API values you need to update in each game
    UserName: null,
    Password: null,
    GameId: null, //"BubbleTopia"
    GameProgress: null, //1
    NumberGamesPlayed: null, //1
    TopScore: null, //424
    LastLevelReached: null, //65
    LastTimeSetting: null, //2
    LastGameScore: null, //5
    TopStar: null, //"three"
    BreathingRate: null, //0.0
    Calmness: null, //0.0
    Date: null, //"29th nov 2016"
    Minutes: null, //"43"
    Correct: 0,
    Incorrect: 0,
    TimeCorrect: 0,
    TimeIncorrect: 0,
    RecordedPauseTimes: [],
    RecordedRoundMatches: [],
    RecordedRoundLevels: [],
    RecordedCorrectTimes: [],
    RecordedInCorrectTimes: [],
    RecordedWordsSelected: [],
    RecordedWordsCorrect : [],
    RecordedWordCount: []
};
...
Brain.Boot.prototype = {
    ...
    create: function () {
        ...
        if (game.device.desktop) {
            ...
            Brain.scaleRatio = window.devicePixelRatio / 3;
        ...
        Brain.GameProgress = 1; //1
        Brain.NumberGamesPlayed = 0; //1
        Brain.TopScore = 0; //424
        Brain.LastLevelReached = 0; //65
        Brain.LastTimeSetting = 0; //2
        Brain.LastGameScore = 0; //5
        Brain.TopStar = 0; //"three"
        Brain.BreathingRate = 0; //0.0
        Brain.Calmness = 0; //0.0
        Brain.Date = GetDate(); //"29th nov 2016"
        Brain.Minutes = 0; //"43"
        ...
        timer.add(0.5 * 1000, preload, this)    
        timer.start() 
    },
...
Brain.FullScreen = function () {
    ...
    if (document.fullscreenEnabled && game.device.iPhone) {
...
Brain.AddTextWithStyle = function (text, position, anchor, style) {
    var game = Brain.game;
    var titleText = game.add.text(position.x, position.y, text, style);

    titleText.anchor.set(anchor.x, anchor.y);
    return titleText;
};
...
Brain.TweenAway = function (sprite) {
    ...
    fade.to({
        alpha: 0
    }, 1 * 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
}
...
Brain.TweenIn = function (sprite, posY) {
    ...
    x.to({
        centerY: posY
    }, 1 * 1000, Phaser.Easing.Quadratic.Out, true, 0, 0, false);
}
...
Brain.TweenScale = function (sprite, width, height, start, duration, repeat) {
    ...
    x.to({
        width: width,
        height: height
    }, duration * 1000, Phaser.Easing.Linear.None, start, 0, repeat, false);
    return x;
}
...
Brain.TweenScale2 = function (sprite, width, height, start, duration) {
    ...
    x.to({
        x: width,
        y: height
    }, duration * 1000, Phaser.Easing.Linear.None, start, 0, 0, false);
}
...
InfoPanel = function (info, size) {
    var game = Brain.game;
    Phaser.Group.call(this, game);
    var graphics = new Phaser.Graphics(game, 0, 0);
    graphics.beginFill(Color.purpleHex);
    graphics.drawRoundedRect(0, 0, 600, 800, 50);
    graphics.endFill();
    var infoPanelTex = graphics.generateTexture();
    this.infoPanel = new Phaser.Sprite(game, 0, 0, infoPanelTex);
    this.info = Brain.AddText(info, 0, 0, Color.white, size, 0.5, 0.5, "bold", 450, "LatoRegular");
    this.info.addFontWeight("normal", 15);
    this.info.addFontWeight("normal", 15);
...
Alert.Create = function (alert, options, x, y, musicPath) {
    ...
    Alert.background = Brain.AddSprite("alert", x, y, 600, 500, 0.5, 0.5, false);
    Alert.background.tint = Brain.gray;
    Alert.background.anchor.set(0.5, 0.5);
    Alert.background.alpha = 0;
    Alert.alert = Brain.AddText(alert, Alert.background.x, Alert.background.top, Color.white, 38, 0.5, 0, "normal", 1000, "LatoRegular");
    ...
    for (i = 0; i < options.length; i++) {
        var button = new Brain.AddButton(options[i].text, 0, y, options[i].callback, options[i].context, "button", 250, 80, 0, 0.5, 21);
    ...
    Alert.options.centerY = this.background.bottom * 0.85;
    ...
    game.load.onFileComplete.add(Alert.FileComplete, this);
...
Alert.FileComplete = function(progress, cachekey, success, totalLoaded, totalFiles)
{
    Alert.spinner.text = "Loading ...";
    Alert.spinner.align = "center";
}

Alert.LoadStart = function () {
    console.log("loading");
    Alert.options.visible = false;
    Alert.spinner = Brain.AddText("Loading", 0, 0, Color.white, 32, 0.5, 05, "normal", 800, "LatoRegular")
    Alert.spinner.centerX = Alert.background.centerX
    Alert.spinner.bottom = Alert.background.bottom - 25
    Alert.spinner.align = "center"
}
...
Spinner = function (x, y) {
    ...
    tween.to({
        rotation: 3
    }, 1 * 1000, Phaser.Easing.Linear.None, true, 0, -1, false);
    return this;
}
