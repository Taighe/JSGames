...
HudManager.Init = function(exercise)
{
    ...
    this.timerSprite = Brain.AddSprite("time-back", topRight.x, topRight.y, 90, 90, 0.5, 0.5, false);
    this.timeSprite = Brain.AddText("0", this.timerSprite.centerX, this.timerSprite.centerY+ 15, Color.white, 38, 0.5, 0.5, "bold", 800, "LatoRegular");
    var scoreText = Brain.AddText("SCORE", topLeft.x, topLeft.y, Color.lightGray, 20, 0, 0.5);
    this.scoreSprite = Brain.AddText("1000", topLeft.x, scoreText.bottom - 5, Color.lightPurple, 48, 0, 0, "bold", 800, "LatoRegular");
    this.countDownTimer = game.time.create(true);
    this.countDownTimer.start();
    this.countDownTimer.pause();
    this.graphics = new Phaser.Graphics(game, 0,0);
    this.graphics.lineStyle(1.5, Color.whiteHex, 1);
    this.graphics.drawCircle(game.width * 0.9, game.height * 0.065, 90)
